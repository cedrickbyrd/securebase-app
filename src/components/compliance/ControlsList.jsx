...
const ALL_CONTROLS = [
  ...
  { id: 'cc1-1', name: 'Integrity and Ethical Values', framework: 'SOC 2 CC1.1', category: 'Control Environment', status: 'passing', evidenceCount: 23, lastChecked: '2 hours ago', description: 'Commitment to integrity and ethical values across all operations' },
  { id: 'cc1-2', name: 'Board Oversight', framework: 'SOC 2 CC1.2', category: 'Control Environment', status: 'passing', evidenceCount: 8, lastChecked: '1 day ago', description: 'Board exercises oversight responsibility for security and compliance' },
  { id: 'cc1-3', name: 'Organizational Structure', framework: 'SOC 2 CC1.3', category: 'Control Environment', status: 'passing', evidenceCount: 15, lastChecked: '1 day ago', description: 'Defined structure with assigned security responsibilities' },
  ...
  { id: 'cc2-1', name: 'Internal Information Requirements', framework: 'SOC 2 CC2.1', category: 'Communication & Information', status: 'passing', evidenceCount: 47, lastChecked: '1 hour ago', description: 'Relevant information identified and communicated for control operation' },
  ...
  { id: 'cc2-3', name: 'External Communication', framework: 'SOC 2 CC2.3', category: 'Communication & Information', status: 'passing', evidenceCount: 12, lastChecked: '6 hours ago', description: 'External parties communicated with regarding commitments' },
  ...
  { id: 'cc3-4', name: 'Change Risk Assessment', framework: 'SOC 2 CC3.4', category: 'Risk Assessment', status: 'passing', evidenceCount: 41, lastChecked: '3 hours ago', description: 'Fraud and change-related risks considered in risk assessment' },
  ...
  { id: 'cc5-3', name: 'Policy Deployment', framework: 'SOC 2 CC5.3', category: 'Control Activities', status: 'passing', evidenceCount: 38, lastChecked: '4 hours ago', description: 'Policies and procedures deployed and communicated to personnel' },
  ...
  // ── A1: Availability ──────────────────────────────────────────────────────
  { id: 'a1-1', name: 'Capacity Planning', framework: 'SOC 2 A1.1', category: 'Availability', status: 'passing', evidenceCount: 52, lastChecked: '1 hour ago', description: 'System capacity planned and monitored to meet availability commitments' },
  { id: 'a1-2', name: 'Environmental Threat Response', framework: 'SOC 2 A1.2', category: 'Availability', status: 'passing', evidenceCount: 38, lastChecked: '2 hours ago', description: 'Environmental threats and incidents monitored and responded to' },
  { id: 'a1-3', name: 'Recovery Testing', framework: 'SOC 2 A1.3', category: 'Availability', status: 'warning', evidenceCount: 6, lastChecked: '30 days ago', warningText: 'Disaster recovery test overdue', description: 'Recovery procedures tested to confirm availability commitments' },
  ...
  // ── P1–P8: Privacy ────────────────────────────────────────────────────────
  { id: 'p1-1', name: 'Privacy Notice', framework: 'SOC 2 P1.1', category: 'Privacy', status: 'passing', evidenceCount: 5, lastChecked: '7 days ago', description: 'Privacy notice communicated to data subjects prior to collection' },
  ...
  { id: 'p4-2', name: 'Retention Policy', framework: 'SOC 2 P4.2', category: 'Privacy', status: 'passing', evidenceCount: 23, lastChecked: '1 day ago', description: 'Personal information retained only as long as necessary' },
  ...
  { id: 'p6-3', name: 'Vendor Privacy Accountability', framework: 'SOC 2 P6.3', category: 'Privacy', status: 'passing', evidenceCount: 18, lastChecked: '2 days ago', description: 'Third-party vendors held accountable for privacy obligations' },
...
export default function ControlsList() {
  ...
  const statusIcon = { passing: '✅', warning: '⚠️', failing: '❌' };
  ...
  return (
    <div className="sb-ControlsList">
      <div className="sb-ControlsList__header">
        ...
        <span className="u-text-muted">{ALL_CONTROLS.length} controls monitored</span>
      </div>

      <div className="sb-ControlsList__filters">
        {filterBtns.map(btn => (
          <button
            key={btn.key}
            className={`sb-ControlsList__filterBtn${activeFilter === btn.key ? ' sb-ControlsList__filterBtn--active' : ''}`}
      ...
      {Object.entries(byCategory).map(([category, controls]) => (
        <div key={category} className="sb-ControlsList__category">
          <h3 className="sb-ControlsList__categoryTitle">
            {category}
            <span className="u-text-muted"> ({controls.length})</span>
          </h3>
...
function ControlCard({ control }) {
  ...
  return (
    <div className={`sb-ControlCard sb-ControlCard--${control.status}`}>
      ...
      {control.description && (
        <p className="sb-ControlCard__description u-text-muted">{control.description}</p>
      )}
