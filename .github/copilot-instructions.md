...
### Technology Stack
- **Frontend (marketing site `src/`):** React 19, Vite 6
- **Frontend (portal `phase3a-portal/`):** React 18, Vite 5
...
9. **Lock file drift** — If CI fails with `npm error code EUSAGE` / `npm ci` sync error, run `npm install` in the affected directory and commit the updated `package-lock.json`. The root and `phase3a-portal/` packages are managed independently.
