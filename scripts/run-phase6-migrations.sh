...
DB_SECRET_ID="${DB_SECRET_ID:-${DB_SECRET_ARN:-securebase/${TARGET_ENV}/rds/admin-password}}"
...
if [ -z "$DB_HOST" ] || [ "$DB_HOST" = "null" ] || [ -z "$DB_USER" ] || [ "$DB_USER" = "null" ] || [ -z "$DB_PASS" ] || [ "$DB_PASS" = "null" ]; then
  echo "Failed to parse DB credentials from Secrets Manager secret: $DB_SECRET_ID" >&2
  exit 1
fi
...
apply_migration() {
  ...
  if psql "$PSQL_CONN" -tA -c "SELECT 1 FROM schema_migrations WHERE version='${version}' LIMIT 1;" | grep -q '^1$'; then
  ...
  psql "$PSQL_CONN" -v ON_ERROR_STOP=1 -c "INSERT INTO schema_migrations(version) VALUES ('${version}') ON CONFLICT (version) DO NOTHING;"
}
