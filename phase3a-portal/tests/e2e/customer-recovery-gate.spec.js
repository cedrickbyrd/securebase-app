...
const RECOVERY_EMAIL = process.env.RECOVERY_CUSTOMER_EMAIL;
...
const MANUAL_INVITE_CONFIRMED = ['true', '1', 'yes'].includes(
  (process.env.RECOVERY_MANUAL_INVITE_CONFIRMED || '').trim().toLowerCase()
);

function assertEnvDefined(name, value) {
...
test.describe('Customer recovery gate', () => {
  test('verifies onboarding recovery journey for one real customer', async ({ page }) => {
    ...
    assertEnvDefined('RECOVERY_CUSTOMER_EMAIL', RECOVERY_EMAIL);
    assertEnvDefined('RECOVERY_PASSWORD', RECOVERY_PASSWORD);
    ...
    try {
      ...
      await test.step('login works via auth API', async () => {
        ...
        expect(loginBody.token, 'login must return JWT').toMatch(/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/);
      ...
      await test.step('relevant compliance view loads', async () => {
        ...
        await expect(page.getByTestId('overall-score-card')).toBeVisible({ timeout: 15000 });
