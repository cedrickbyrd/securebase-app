...
-- Index for fast entitlement lookups on every auth request
CREATE INDEX IF NOT EXISTS idx_customers_marketplace_customer_id
  ON customers(marketplace_customer_id);

-- Metering records table — immutable log of every BatchMeterUsage call
